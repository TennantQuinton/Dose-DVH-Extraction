        ...
        public void ExportDoseAsExcel(List<PlanQueries> queryList)
        {
			...
			foreach (PlanQueries queriesByPlan in queryList)
			{
				...
				if (newWorksheet)
				{
					...
					colIndex++;
					if (queriesByPlan.GetPlanID().Contains("BRAI"))
					{
						xlSheet.Cells[rowMainIndex, colIndex] = "Plan Information";
						xlSheet.Cells[rowSubIndex, colIndex] = "SRS Target";
						colIndex++;
					}
				...
				colIndex++;
				xlSheet.Cells[rowToWrite, colIndex] = "null";//patient.DateOfBirth;
				colIndex++;
				...
				{
					if (queriesByPlan.GetPlanID().ToUpper().Contains("BRAI") || queriesByPlan.GetPlanID().ToUpper().Contains("SRS"))
					{
						if (queriesByPlan.GetPlanID().Contains("1"))
						{
							num = "1";
						}
						else if (queriesByPlan.GetPlanID().Contains("2"))
						{
							num = "2";
						}
						else if (queriesByPlan.GetPlanID().Contains("3"))
						{
							num = "3";
						}
						else if (queriesByPlan.GetPlanID().Contains("4"))
						{
							num = "4";
						}
						else if (queriesByPlan.GetPlanID().Contains("5"))
						{
							num = "5";
						}
						else if (queriesByPlan.GetPlanID().Contains("6"))
						{
							num = "6";
						}
						else if (queriesByPlan.GetPlanID().Contains("7"))
						{
							num = "7";
						}

						xlSheet.Cells[rowToWrite, colIndex] = $"GTV{num}";
						colIndex++;
					}
